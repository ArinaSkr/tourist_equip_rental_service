...
class RegistrationDialog(QDialog):
    ...
    def register(self):
        ...
        if len(password) < 4:
            QMessageBox.warning(
                self, "Ошибка", "Пароль должен содержать не менее 4 символов"
            )
            return
        ...
        try:
            ...
            if new_login and new_login != self.user.login:
                ...
                if not current_password:
                    QMessageBox.warning(
                        self, "Ошибка", "Для смены логина нужен текущий пароль"
                    )
                    return
            ...
            if new_password:
                if not current_password:
                    QMessageBox.warning(
                        self, "Ошибка", "Для смены пароля нужен текущий пароль"
                    )
                    return
                if len(new_password) < 4:
                    ...
                    return
                self.user.change_info(
                    "пароль", new_password, old_password=current_password
                )
                changes_made = True
...
class MainWindow(QMainWindow):
    ...
    def create_offers_tab(self):
        ...
        edit_btn.clicked.connect(
            lambda: self.show_offer_details(self.offers_list.currentItem())
        )
    ...
    def update_equipment_list(self):
        ...
        for item in available_items:
            list_item = QListWidgetItem(f"{item.name} - {item.price_per_day} руб/день")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.rental.remove_booking_by_item(equipment, self.user):
                QMessageBox.information(
                    self, "Успех", f"Бронирование '{equipment.name}' удалено"
                )
                self.update_bookings_list()
                self.update_equipment_list()
            else:
...
class EQUIPleaseApp(QWidget):
    ...
    def show_main_window(self):
        ...
        self.main_window = MainWindow(
            self.current_user, self.users, self.catalog, self.rental
        )
