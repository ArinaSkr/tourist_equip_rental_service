            ...
            case "нет":
                while actual_user is None:
                    ...
                    if users.check_duplicate(user_full_name, user_tel):
                        choice = input(
                            "Пользователь с такими данными уже существует. Хотите войти? (да/нет): "
                        )
                        if choice.lower() == "да":
                            login = input("Введите логин: ")
                            password = get_password_input("Введите пароль: ")
                    ...
                    if users.find_user(user_login):
                        ...
                        continue
                    user_password = get_password_input(
                        "Введите пароль(не менее 4 символов): "
                    )
                    try:
                        user = users.add_user(
                            user_full_name, user_tel, user_login, user_password
                        )
                        actual_user = user
                        break
        ...
        match menu:
            case 1:
                ...
                if available_items:
                    ...
                    input_str = input(
                        "Вы можете открыть карточку любого товара. Для этого введите его название: "
                    )
                    if input_str != "":
                        ...
                        if found_item:
                            for item in found_item:
                                print(item)
                                book_choice = input(
                                    "Хотите забронировать этот товар? (да/нет):"
                                )
                    ...
                    print(
                        "Сейчас нет доступного снаряжения, но вы можете первым его добавить"
                    )
            ...
            case 2:
                ...
                if choice_change.lower() in "да":
                    what_change = input(
                        "Какую информацию хотите изменить?(ФИО, телефон, логин, пароль): "
                    )
                    if what_change.lower() in ["фио", "телефон"]:
                        ...
                        old_password = get_password_input(
                        ...
                        if users.find_user(new_login):
                            ...
                            try:
                                actual_user.change_info(
                                    "логин", new_login, old_password=old_password
                                )
                        ...
                        try:
                            actual_user.change_info(
                                "пароль", new_password, old_password=old_password
                            )
                        except WrongPasswordError as e:
                            print(f"Ошибка: {e}")
            case 3:
                ...
                new_item = EquipmentItem(
                    name, price, deposit, description, owner=actual_user
                )
            ...
            case 4:
                ...
                if user_bookings:
                    ...
                    if calc_choice.lower() in "да":
                        try:
                            days = int(
                                input("Введите количество дней для расчета стоимости: ")
                            )
                            total_price = rental_service.full_rent_price(
                                days, actual_user, catalog
                            )
                    ...
                    if change_choice.lower() in "да":
                        try:
                            item_number = int(
                                input(
                                    "Введите номер товара для удаления (или 0 для отмены): "
                                )
                            )
                            if item_number == 0:
                                continue
                                ...
                                if rental_service.remove_booking_by_item(
                                    item_to_remove, actual_user
                                ):
                                    print(
                                        f'Товар "{item_to_remove.name}" удален из бронирований'
                                    )
                                else:
                ...
                while True:
                    ...
                    if user_offers:
                        ...
                        for item in user_offers:
                            print(f"- {item.name}: {item.price_per_day} руб/день")
                        choice_change_item = input(
                            "Если хотите отредактировать/удалить объявление о снаряжении, введите его название: "
                        )
                        ...
                        if target_item:
                            action_item = input(
                                "Что хотите сделать? (удалить/изменить/назад): "
                            )
                            if action_item.lower() == "удалить":
                                ...
                                what_change = input(
                                    "Какую информацию хотите изменить?(название, описание, цена, залог): "
                                ).lower()
                                if what_change in [
                                    "название",
                                    "описание",
